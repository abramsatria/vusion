...
async function cekKey(req, res, next) {
    ...
    if(db === null) {
		return res.json({ status : false, creator : `${creator}`, message : "[!] Apikey Tidak Ada"})  
				...
				return res.json({ status : false, creator : `${creator}`, message : "[!] Verifikasi Email Terlebih Dahulu!"})  
...
router.get('/api/maker/darkness', cekKey, async (req, res) => {
	...
	var n = isNumber(no)
	if ( !img ) return res.json({ status : false, creator : 'Alip', message : "[!] cek kembali url image"}) 
	if ( !n ) return res.json({ status : false, creator : 'Alip', message : "[!] parameter number "}) 
...
router.get('/api/maker/resize', cekKey, async (req, res) => {
	...
	if ( !img ) return res.json({ status : false, creator : 'Alip', message : "[!] cek kembali url image"}) 
	if ( !wid ) return res.json({ status : false, creator : 'Alip', message : "[!] parameter width number "}) 
	if ( !hei ) return res.json({ status : false, creator : 'Alip', message : "[!] parameter height number "}) 
...
router.get('/api/maker/trigger', cekKey, async (req, res) => {
	...
	if (!text ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter url"})
	var img = await isImageURL(text)
	if ( !img ) return res.json({ status : false, creator : 'Abram Satria', message : "[!] cek kembali url image"}) 
...
router.get('/api/maker/welcome1', cekKey, async (req, res, next) => {
    ...
    if (!name ) return res.json({ status : false, creator : 'Abram Satria', message : "[!] masukan parameter name"})  
	if (!grup ) return res.json({ status : false, creator : 'Abram Satria', message : "[!] masukan parameter gpname"})  
    if (!member ) return res.json({ status : false, creator : 'Abram Satria', message : "[!] masukan parameter member"})  
	if (!pp ) return res.json({ status : false, creator : 'Abram Satria', message : "[!] masukan parameter pp"})  
    if (!bg ) return res.json({ status : false, creator : 'Abram Satria', message : "[!] masukan parameter bg"})  

	if ( !imgpp ) return res.json({ status : false, creator : 'Abram Satria', message : "[!] cek kembali url image pp"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'Abram Satria', message : "[!] cek kembali url image bg"}) 
...
router.get('/api/maker/goodbye1', cekKey, async (req, res, next) => {
	...
	if ( !imgpp ) return res.json({ status : false, creator : 'Abram Satria', message : "[!] masukan parameter pp Link pp dengan betul"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'Abram Satria', message : "[!] masukan parameter pp Link bg dengan betul"}) 
...
router.get('/api/linkshort/tinyurl', cekKey, async (req, res, next) => {
	...
	if (!islink ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter url "})  
...
router.get('/api/info/emoji', cekKey, async (req, res, next) => {
	...
	if (!emoji1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter emoji"})
      var hasil = emoji.get(emoji1)
       if (hasil == null) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter emoji dengan 1 emoji "})
           limitapikey(req.query.apikey)
           res.json({
			status: true,
	        creator: `${creator}`,
